function! high#light#indent#define(settings)
  ...
  for i in range(lighter._start_level, lighter._levels+1)
    call high#main#AddLighter(extend(high#main#Clone(lighter), {'pattern_to_eval': '"^\\( \\{".&sw."}\\|\\t\\)\\{'.i.'}\\zs\\( \\{'.(lighter._size > 0 ? lighter._size : '".&sw."').'}\\|\\t\\)"', 'hlgroup': lighter[i%2 ? '_hlgroupB' : '_hlgroupA']}))
  endfor
endfunction
