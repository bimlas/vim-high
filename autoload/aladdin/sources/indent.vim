function! aladdin#sources#indent#define(settings)
  ...
  for i in range(preconfigured.start_level, preconfigured.levels+1)
    call add(obj, extend(preconfigured._Clone(), {'_pattern_to_eval': '"^\\( \\{".&sw."}\\|\\t\\)\\{'.i.'}\\zs\\( \\{'.(preconfigured.size > 0 ? preconfigured.size : '".&sw."').'}\\|\\t\\)"', 'hlgroup': preconfigured[i%2 ? 'hlgroupB' : 'hlgroupA']}))
  endfor
